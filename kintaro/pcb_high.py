...
if (GPIO.input(pcb_components["POWER"]) == GPIO.HIGH) and GPIO.input(pcb_components["CHECK_PCB"]) == GPIO.HIGH:
    os.system("poweroff")
...
if GPIO.input(pcb_components["CHECK_PCB"])==GPIO.HIGH: #check if there is an pcb and if there is then attach the interrupts
    toggle(0.5)
